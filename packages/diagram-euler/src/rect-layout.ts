...
import type { PlaceholderRect, EntityRect, RectLayoutResult } from './types.ts'
import type {
  Box,
  LayoutInput,
  EntityClass,
 ...
 */
export function computeRectLayout(input: LayoutInput): RectLayoutResult {
 ...
 */
export function probeRectFaithfulness(input: LayoutInput): Array<string> {
  const drawable = input.entities.filter((entity) => entity.special === 'none')
